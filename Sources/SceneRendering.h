...
struct FSortedShadowMaps
{
	...
	TArray<FSortedShadowMapAtlas, SceneRenderingAllocator> CSMScrollingCacheAtlases;
	...
	int64 ComputeMemorySize() const
	{
		...
		for (int i = 0; i < CSMScrollingCacheAtlases.Num(); i++)
		{
			MemorySize += CSMScrollingCacheAtlases[i].RenderTargets.ComputeMemorySize();
		}
		// @StarLight code - END CSM Scrolling optmization, edit by wanghai
