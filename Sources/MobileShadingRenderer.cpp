...
void FMobileSceneRenderer::InitViews(FRHICommandListImmediate& RHICmdList)
{
	...
	//@StarLight code - BEGIN GPU-Driven, Added by yanjianhong
	if (bUseMobileGpuDriven(Views[0])) {
	...
	//@StarLight code - BEGIN Mobile Cluster Lighting For Mobile By wanghai
	if (!bDeferredShading)
	{
		MobileComputeLightGrid(RHICmdList);
	}
	//@StarLight code - END Mobile Cluster Lighting For Mobile By wanghai
...
FRHITexture* FMobileSceneRenderer::RenderForward(FRHICommandListImmediate& RHICmdList, const TArrayView<const FViewInfo*> ViewList)
{
	...
	// @StarLight code - BEGIN CSM Scrolling optmization, edit by wanghai
	if (Scene->ProjectedCSMScrollingInfos.Num() > 0)
	{
		static auto* CVarMobileUseCSMScrollingOptmization = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.Mobile.UseCSMScrollingOptmization"));
		bool bUseCSMScrolling = CVarMobileUseCSMScrollingOptmization->GetValueOnRenderThread() == 1;

		if (bUseCSMScrolling)
		{
			Scene->ProjectedCSMScrollingInfos[0].DrawScrollingCacheToViewport(RHICmdList, this, View);
		}
	}
	// @StarLight code - END CSM Scrolling optmization, edit by wanghai
	RHICmdList.ImmediateFlush(EImmediateFlushType::DispatchToRHIThread);
