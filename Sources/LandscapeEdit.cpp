...
void ULandscapeComponent::UpdateMaterialInstances_Internal(FMaterialUpdateContext& Context)
{
	...
	for (auto It = MaterialPerLOD.CreateConstIterator(); It; ++It)
	{
		...
		if (CombinationMaterialInstance != nullptr)
		{
			...
			for (int32 i = 0; i < GetLandscapeProxy()->WholeWeightmaps.Num(); i++)
			{
				MaterialInstance->SetTextureParameterValueEditorOnly(FName(*FString::Printf(TEXT("VirtualTextureWeightmap%d"), i)), GetLandscapeProxy()->WholeWeightmaps[i]);
			}
...
#define MAX_HEIGHTMAP_TEXTURE_SIZE 2048
//@StarLight code - LandscapeGpuRender, Added by yanjianhong
...
void ALandscapeProxy::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(ALandscapeProxy, bUseMaterialPositionOffsetInStaticLighting))
	{
		...
		//@StarLight code - BEGIN Remove landscape lightmap
		PropertyName == FName(TEXT("LightmapType")))
		//@StarLight code - END Remove landscape lightmap
	{
...
void ULandscapeComponent::GeneratePlatformPixelData()
{
	...
	if (!GIsEditor)
	{
		...
		for (int32 MaterialIndex = 0; MaterialIndex < MobileCombinationMaterialInstances.Num(); ++MaterialIndex)
		{
			...
			//@StarLight code - BEGIN Merge All Component's Weightmap to One Weightmap, Added by zhuyule
			for (int TextureIdx = 0; TextureIdx < GetLandscapeProxy()->WholeWeightmaps.Num(); TextureIdx++)
			{
				NewMobileMaterialInstance->SetTextureParameterValue(FName(*FString::Printf(TEXT("VirtualTextureWeightmap%d"), TextureIdx)), GetLandscapeProxy()->WholeWeightmaps[TextureIdx]);
			}
			//@StarLight code - END   Merge All Component's Weightmap to One Weightmap, Added by zhuyule
	...
	else
	{
		...
		for (auto It = MaterialPerLOD.CreateConstIterator(); It; ++It)
		{
			...
			for (int TextureIdx = 0; TextureIdx < GetLandscapeProxy()->WholeWeightmaps.Num(); TextureIdx++)
			{
				NewMobileMaterialInstance->SetTextureParameterValueEditorOnly(FName(*FString::Printf(TEXT("VirtualTextureWeightmap%d"), TextureIdx)), GetLandscapeProxy()->WholeWeightmaps[TextureIdx]);
			}
			//@StarLight code - END   Merge All Component's Weightmap to One Weightmap, Added by zhuyule
...
void ALandscapeProxy::GenerateWeightmap()
{
	...
	WholeWeightmaps.Empty();

	TSet<FName> LayerNames;
	GetAllMobileRelevantLayerNames(LayerNames, GetLandscapeMaterial()->GetMaterial());

	int32 WeightmapsNum = (LayerNames.Num() - 1) / 4 + 1;
	uint8 MaxMip = 0;
	...
	if (ComponentCount > 0)
	{
		...
		if (FirstLandscape != nullptr)
		{
			...
			for (int32 WeightmapsIndex = 0; WeightmapsIndex < WeightmapsNum; WeightmapsIndex++)
			{
				UTexture2D* WholeWeightmap = CreateLandscapeTexture(WeightmapSize, WeightmapSize, TEXTUREGROUP_Terrain_Weightmap, TSF_BGRA8, nullptr, false);
				...
				MaxMip = FMath::CeilLogTwo(TileWeightmapSize) + 1;
				...
				WholeWeightmaps.Add(WholeWeightmap);
			}
			...
			for (int32 Index = 0; Index < ComponentCount; Index++)
			{
				...
				if (LandscapeComponent != nullptr)
				{
					...
					{
						int32 WeightmapsIndex = ValidLayerNum / 4;
						...
						for (auto& Allocation : ComponentWeightmapLayerAllocations)
						{
							if (Allocation.GetLayerName() == LayerName)
							{
								...
								LandscapeData.CopyTextureFromWeightmap(WholeWeightmaps[WeightmapsIndex], ValidLayerNum % 4, LandscapeComponent, Allocation.LayerInfo, Start, End, MaxMip);
							}
						}
						ValidLayerNum += 1;
					}
			...
			GDisableAutomaticTextureMaterialUpdateDependencies = true;
			for (int TextureIdx = 0; TextureIdx < WholeWeightmaps.Num(); TextureIdx++)
			{
				UTexture* Texture = WholeWeightmaps[TextureIdx];
				Texture->PostEditChange();
