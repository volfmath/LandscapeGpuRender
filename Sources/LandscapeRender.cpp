...
FVector4 FLandscapeComponentSceneProxy::GetSpecialValue() const
{
	...
	bool ShouldCache(EShaderPlatform Platform, const FShaderType* ShaderType, const FVertexFactoryType* VertexFactoryType) const override
	{
		...
		if (VertexFactoryType)
		{
			...
			else
			{
				...
				static const FName LandscapeGpuRenderVertexFactory = FName(TEXT("FLandscapeGpuRenderVertexFactory"));
				//@StarLight code - LandscapeGpuRender, Added by yanjianhong
				...
				if (VertexFactoryType->GetFName() == LandscapeVertexFactory ||
					...
					VertexFactoryType->GetFName() == LandscapeGpuRenderVertexFactory)
				{
