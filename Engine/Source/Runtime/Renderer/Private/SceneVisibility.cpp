		...
		for (int32 Index = 0; Index < Input.NumPrims; Index++)
		{
			...
			if (bTranslucentRelevance && !bEditorRelevance && ViewRelevance.bRenderInMainPass)
			{
				if (View.Family->AllowTranslucencyAfterDOF())
				{
					...
					if (ViewRelevance.bDownSampleSeparateTranslucency) {
						TranslucentPrimCount.Add(ETranslucencyPass::TPT_TranslucencyDownSampleSeparate, ViewRelevance.bUsesSceneColorCopy, ViewRelevance.bDisableOffscreenRendering);
					}
					//End
					...
					if (ViewRelevance.bDrawRelevance)
					{
							...
							&& ViewRelevance.bRenderInMainPass)
						{
							if (View.Family->AllowTranslucencyAfterDOF())
							{
								...
								//Yjh Created By 2020-7-25
								//for static DrawCommand
								if (ViewRelevance.bDownSampleSeparateTranslucency) {
									DrawCommandPacket.AddCommandsForMesh(PrimitiveIndex, PrimitiveSceneInfo, StaticMeshRelevance, StaticMesh, Scene, bCanCache, EMeshPass::TranslucencyDownSampleSeparate);
								}
								//End
...
void ComputeDynamicMeshRelevance(EShadingPath ShadingPath, bool bAddLightmapDensityCommands, const FPrimitiveViewRelevance& ViewRelevance, const FMeshBatchAndRelevance& MeshBatch, FViewInfo& View, FMeshPassMask& PassMask, FPrimitiveSceneInfo* PrimitiveSceneInfo, const FPrimitiveBounds& Bounds)
{
		...
		&& ViewRelevance.bRenderInMainPass)
	{
		if (View.Family->AllowTranslucencyAfterDOF())
		{
			...
			//for dynamicMesh
			if (ViewRelevance.bDownSampleSeparateTranslucency) {
				PassMask.Set(EMeshPass::TranslucencyDownSampleSeparate);
				View.NumVisibleDynamicMeshElements[EMeshPass::TranslucencyDownSampleSeparate] += NumElements;
			}
			//End
	...
	FGlobalDynamicIndexBuffer& DynamicIndexBuffer, FGlobalDynamicVertexBuffer& DynamicVertexBuffer, FGlobalDynamicReadBuffer& DynamicReadBuffer)
{
	...
	//@StarLight code - BEGIN LandScapeInstance, Added by yanjianhong
	{
		for (int32 ViewExt = 0; ViewExt < ViewFamily.ViewExtensions.Num(); ++ViewExt)
		{
			...
			{
				ViewFamily.ViewExtensions[ViewExt]->PreComputeViewVisibility_RenderThread(RHICmdList, Views[0]);
			}
		}
	}
	//@StarLight code - END LandScapeInstance, Added by yanjianhong
...
bool FSceneRenderer::DoHZBOcclusion() const{
	return GHZBOcclusion == 2;
}
