...
class FLandscapeComponentClusterDeriveData {
public:

	/** The compressed Landscape component data for mobile rendering. Serialized to diskOn device, freed once it has been decompressed. */
	TArray<uint8> UnCompressedClusterLandscapeData;

	///** Cached render data. Only valid on device. */
	//TSharedPtr<FLandscapeMobileClusterRenderData, ESPMode::ThreadSafe > CachedClusterRenderData;

	//This function is only called by SceneProxy construct
	TSharedPtr<FLandscapeMobileClusterRenderData, ESPMode::ThreadSafe> GetRenderData();

	bool HasValidPlatformData() const
	{
		return UnCompressedClusterLandscapeData.Num() != 0;
	}

	/** Initializes the compressed data from an uncompressed source. */
	void InitializeFromUncompressedClusterData(const TArray<uint8>& UncompressedData);
	...
	friend FArchive& operator<<(FArchive& Ar, FLandscapeComponentClusterDeriveData& Data);
};
...
public:
	...
	//@StarLight code - BEGIN LandScapeInstance, Added by yanjianhong
	FLandscapeComponentClusterDeriveData ClusterPlatformData;
	//@StarLight code - END LandScapeInstance, Added by yanjianhong
	...
	//@StarLight code - BEGIN layer's ratio below the smallest value will be discarded, Added by zhuyule
	LANDSCAPE_API void DeleteLayerBelowSmallestRatio(float RatioValue);
	//@StarLight code - BEGIN layer's ratio below the smallest value will be discarded, Added by zhuyule
	...
	//@StarLight code - BEGIN LandScapeInstance, Added by yanjianhong
	void GeneratePlatformClusterData();
	//@StarLight code - END LandScapeInstance, Added by yanjianhong
