...
void FLandscapeRenderSystem::ComputeSectionPerViewParameters(
	...
	FMatrix RealViewProjectionMatrix
)
{
...
FLandscapeRenderSystem::FComputeSectionPerViewParametersTask::FComputeSectionPerViewParametersTask(FLandscapeRenderSystem& InRenderSystem, const FSceneView* InView)
	...
	, RealViewProjectionMatrix(InView->ViewMatrices.GetViewMatrix() * InView->ViewMatrices.GetProjectionMatrix())
{
}
...
// @StarLight code - BEGIN Fix the Landscape Occlusion Culling Bug
	int32 TotalSubSection = NumSubsections * NumSubsections;
	uint16 SingleSubSectionVertices = NumOccluderVertices / TotalSubSection;

	uint16 NumLineQuads = ((uint16)FMath::Sqrt(SingleSubSectionVertices) - 1);
	uint16 NumLineVtx = NumLineQuads + 1;
	check(NumLineVtx*NumLineVtx == SingleSubSectionVertices);

	int32 NumTris = NumLineQuads * NumLineQuads * 2;
	int32 NumIndices = NumTris * 3 * TotalSubSection;
	...
	for (int32 SectionIndex = 0; SectionIndex < TotalSubSection; ++SectionIndex) {
